...
**Inventory (optional):** run [`../scripts/export-winget.ps1`](../scripts/export-winget.ps1) and [`../scripts/run-windows-inventory.ps1`](../scripts/run-windows-inventory.ps1) from the repo on Windows; outputs go under `imports/` (mostly gitignored — see [scripts README — The imports directory](../scripts/README.md#the-imports-directory)). Optionally [`../scripts/list-windows-shortcuts.ps1`](../scripts/list-windows-shortcuts.ps1) **`-OutFile ..\imports\start-menu-shortcuts.txt`**; optionally [`../scripts/capture-dism-features.ps1`](../scripts/capture-dism-features.ps1) (DISM / UAC on non-admin). Update [CAPTURE-MANIFEST.txt](../imports/CAPTURE-MANIFEST.txt) if you use it as a small run log. Re-run after **bulk** app changes.
