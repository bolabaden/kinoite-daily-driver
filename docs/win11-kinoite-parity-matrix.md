...
Refresh **`imports/`** with the scripts in [scripts README — The imports directory](../scripts/README.md#the-imports-directory): **`export-winget.ps1`**, **`run-windows-inventory.ps1`**, and optionally **`list-windows-shortcuts.ps1 -OutFile …\imports\start-menu-shortcuts.txt`**; optional **`capture-dism-features.ps1`** (DISM / UAC on non-admin; add **-PassThru** to return the captured text in the shell). **[`imports/CAPTURE-MANIFEST.txt`](../imports/CAPTURE-MANIFEST.txt)** is a small, committable index you can update by hand. Narrative inventory and app categories live in **`docs/app-mapping.md`**.

**Optional index:** **`imports/CAPTURE-MANIFEST.txt`** and [README.md — Where to start](../README.md#where-to-start). Older files (e.g. `winget-list.txt`, ARP CSV) may still exist on disk from prior runs; they are **not** regenerated by scripts in this repo anymore.
...
| Windows optional features (WSL, Hyper-V, VMs, .NET) | `scripts/capture-dism-features.ps1` (**740** / UAC on non-admin; optional **-PassThru** to return text) | `imports/dism-features.txt` (gitignored) — includes header, subset lines, and full `dism /Online /Get-Features` text |
