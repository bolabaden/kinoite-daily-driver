...
| Windows optional features (WSL, Hyper-V, VMs, .NET) | `scripts/capture-dism-features.ps1` (UAC if DISM return **740** on non-admin) | `imports/dism-features.txt` (gitignored) — includes header, subset lines, and full `dism /Online /Get-Features` text |
