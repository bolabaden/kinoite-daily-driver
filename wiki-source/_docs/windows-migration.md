...
Refresh **`imports/`** from a Windows PowerShell session using **[`scripts/windows-inventory.ps1`](../scripts/windows-inventory.ps1)**: with **no** `-Winget`, `-CimWsl`, `-StartMenu`, or `-Dism` switch it runs **all** four — winget export, CIM+WSL+podman text, Start Menu and Desktop `.lnk` paths, and `dism /Online /Get-Features` (may **UAC** on non-admin; **`-DismPassThru`** returns the generated document text in the shell). For subsets, pass e.g. **`-Dism`**, **`-Winget`**, **`-CimWsl`**, or **`-StartMenu`**; **`-DismOutputPath`**, **`-StartMenuOutFile`**, and **`-OutDir`** target paths under `imports/`. See [scripts README — The imports directory](../scripts/README.md#the-imports-directory) and [`imports/CAPTURE-MANIFEST.txt`](../imports/CAPTURE-MANIFEST.txt) (optional committable index). Category narrative and the long-tail **TSV** are **below** on this page (no separate `app-mapping.md`).
