...
| **Every plan `todos` id** | **Done** | [docs/plan-frontmatter-coverage.md](docs/plan-frontmatter-coverage.md) — full **Appendix A** lists each `doc-*` → `docs/…` path; [plan-stipulated-file-tree.md](docs/plan-stipulated-file-tree.md) matches plan **Workspace path** block |
