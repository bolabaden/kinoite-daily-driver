...
| `winget export` refresh | **Done 2026-04-25** | `imports/winget-export-20260425T171450.json` (gitignored) — `export-winget.ps1` resolves `winget.exe` under `WindowsApps` and merges `Machine`/`User` `PATH` for **minimal** shells. Many `not available from any source` / Steam rows = **expected** |
...
| `run-windows-inventory.ps1` | **Done 2026-04-25** | `imports/windows-inventory-20260425T171455.txt` — CIM, `wsl` + best-effort **podman** (stderr if VM/connection is not up) |
...
**Last update:** 2026-04-25 (exhaustive: `plan-frontmatter-coverage` manifest, `winget` + CIM inventory refresh to `*T171450` / `*T171455`).
